...
- 최신 승인 Web UX 목업: `.code-planner/02-planning/mockups/phase-3-ui-integrated-mockup.html`
- 최신 Onboarding 목업: `.code-planner/02-planning/mockups/phase-3-page-onboarding-mockup.html`
- 최신 목업 승인 기록: `.code-planner/02-planning/review/phase-3-ux-mockup-approval.md`
- 이전 Web Review 목업: `.code-planner/02-planning/mockups/phase-3-web-review-mockup.html`은 superseded로 취급한다.
...
목표: 단일 관리자용 Onboarding, Dashboard, Inbox, Mapping, Vault Browser, Wiki, Settings 구현.

최신 승인된 목업과 UX spec을 기준으로 구현한다. 구현 중 UX 방향을 임의로 변경하면 안 된다.

UX 변경 건 지시서:

- `.code-planner/02-planning/handoff/phase-3-ux-change-plan.md`
- 이 문서는 전체 Phase 3 재계획이 아니라 mobile side menu와 Mapping existing wiki action 수정 지시만 담는다.

정상 운영 잔여 수정 계획:

- `.code-planner/02-planning/handoff/phase-3-normal-operation-gap-plan.md`
- 이 문서는 Phase 3 Web UI가 화면 목업 수준을 넘어 정상 운영되기 위해 남은 onboarding gating, backend 연결, 실제 pipeline, mapping 반영, prompt/connection test gap을 정리한다.

- Integrated HTML: `.code-planner/02-planning/mockups/phase-3-ui-integrated-mockup.html`
- Onboarding HTML: `.code-planner/02-planning/mockups/phase-3-page-onboarding-mockup.html`
- Approval lock: `.code-planner/02-planning/review/phase-3-ux-mockup-approval.md`

최신 필수 UX:

- Navigation: `Onboarding | Dashboard | Inbox | Mapping | Vault | Wiki | Settings`
- Mobile navigation: 상단 메뉴 나열 금지. 좌측 사이드 메뉴 버튼(☰)으로 drawer를 열고, drawer 안에서 top-level 메뉴와 Mapping/Settings 세부 메뉴에 진입한다.
- Logout: top-level nav가 아니라 향후 사용자 메뉴 dropdown에 배치한다. 사용자 메뉴가 아직 없으면 Settings > Auth 임시 액션으로만 제공한다.
- Dashboard: `Inbox / Mapping / Wiki / Vault / Issues` 카드와 Needs attention
- Inbox: upload/text/scan, 자동 processing, log, completed result record
- Mapping: `Page 검증 → Page Mapping → Relationship 검증 → 오류/에러`, 최종 confirm은 3단계 후
- Mapping existing wiki action: similarity-ranked 기존 wiki match 목록, `Use this wiki`/`Switch`, `Add to selected wiki`, `Merge into selected wiki`, `Create new`, `Confirm mapping` CTA를 반드시 제공한다.
- Mobile Mapping: 단일 column stepper + 하단 sticky CTA. 모바일에서도 `Reject + Retry`, `Confirm` 핵심 결정을 허용한다.
- Mobile Mapping sticky CTA에도 기존 wiki 매핑 액션이 포함되어야 한다.
- Wiki: 목차/list + Markdown viewer, graph는 본문 맨 아래, mobile 목차 drawer/search
- Vault Browser: read-only folder/file/content viewer, Markdown viewer, 수정/새 페이지 제외
- Settings LLM: Onboarding과 동일한 기본 설정, route는 advanced, concurrency 기본 1 최대 3
- Settings Prompt: Phase 2 default prompt 표시, version history, rollback, test→confirm
- Errors: 별도 top-level Error 메뉴 없이 Dashboard issues + 각 페이지 contextual error

UX lock:

- Build가 위 승인 방향을 구현할 수 없는 경우, 임의 변경하지 말고 사용자 승인을 요청한다.
- Legacy `.code-planner/02-planning/mockups/phase-3-web-review-mockup.html` 구조로 회귀하면 안 된다. 구현 기준은 integrated mockup이다.
